...
dl <- tempfile()
download.file("https://www.ine.gov.py/microdatos/register/EPHC-ANUAL/REG01_EPHC_ANUAL_2017.SAV", dl)

dl <- tempfile()
download.file("https://www.ine.gov.py/microdatos/register/EPHC-ANUAL/REG01_EPHC_ANUAL_2018.SAV", dl)
...
Data <- EPHC_2017 %>% 
  select(c("DPTO","AREA","P02","P06","ED01","ED0504","ED10","e01aimde", "ipcm", "FEX")) %>% 
...
names(Data) <- c("Department","Area", "Age", "Sex", "Language",
                 "Grade","Cause","Income", "Income_pc","Fex","Drop_out")
...
rm(EPHC_2017,EPHC_2018, index, dl)
#======================================================================================================
#~~~~~~~~~~~~~~~~~~#
# Data Exploration #
#~~~~~~~~~~~~~~~~~~#
...
# View some tables
data.frame("Mujeres"=sum(ifelse(Data$Sex=="Mujeres", Data$Fex, 0)), 
           "Hombres"=sum(ifelse(Data$Sex=="Hombres", Data$Fex, 0)))

data.frame("Urbana"=sum(ifelse(Data$Area=="Urbana", Data$Fex, 0)), 
           "Rural"=sum(ifelse(Data$Area=="Rural", Data$Fex, 0)))

data.frame("Desertor"=sum(ifelse(Data$Drop_out=="Desertor", Data$Fex, 0)), 
           "No_Desertor"=sum(ifelse(Data$Drop_out=="No_Desertor", Data$Fex, 0)))

Datades <- data.frame("Desertor"=sum(ifelse(Data$Drop_out=="Desertor", Data$Fex, 0)), 
                      "No_Desertor"=sum(ifelse(Data$Drop_out=="No_Desertor", Data$Fex, 0)))

Valdes <- data.frame("Desertor"=sum(ifelse(Validation$Drop_out=="Desertor", Validation$Fex, 0)), 
                     "No_Desertor"=sum(ifelse(Validation$Drop_out=="No_Desertor", Validation$Fex, 0)))


Desertor_Data <- Datades[1]/Datades[2]*100
Desertor_Val <- Valdes[1]/Valdes[2]*100
Desertor_Data
Desertor_Val
...
Dep_Des <- Data %>% filter(Data$Drop_out=="Desertor") %>% 
  group_by(Department) %>% summarise(Desertor=sum(Fex))

Dep_NoDes <- Data %>% filter(Data$Drop_out=="No_Desertor") %>% 
  group_by(Department) %>% summarise(No_Desertor=sum(Fex))

t(Dep_Des)

Dep_graph <- merge(Dep_Des, Dep_NoDes, by=intersect(names(Dep_Des), names(Dep_NoDes)))
...
# Saw the Results
data.frame(Model_Results) %>% mutate_if(is.numeric, round, digits=4)
#==============================================================================================
...
Model_Results <- data.frame(Model=models,
                               F1_Score=as.numeric(Matrix_Models),
                               Accuracy=as.numeric(Matrix_Acc)) %>% arrange(desc(F1_Score))

# Saw the Results
Model_Results %>% mutate_if(is.numeric, round, digits=4)
#==============================================================================================
...
Model_Results <- bind_rows(Model_Results,
                           data.frame(Model="fda", 
                                  F1_Score = as.numeric(Matrix_fda$byClass["F1"]),
                                  Accuracy = as.numeric(Matrix_fda$byClass["Balanced Accuracy"]))) %>% 
...
# Saw the Results
data.frame(Model_Result) %>% mutate_if(is.numeric, round, digits=4)
#==============================================================================================
...
# we only get a precision of 0.2703 so our model clearly improves the prediction of 
# this dataset with 0.6275 of precision.
#======================================================================================================